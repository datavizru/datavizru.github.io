# Jobless rates fall in nine states but stall in Michigan

## Unemployment, %, seasonally adjusted, Jan–Oct 2025

### Visualization brief

Chart 10 state series over time. Highlight the Midwest outlier and the national downward trend.

### Data

| Month | State | Unemployment_pct |
| --- | --- | --- |
| 2025-01 | California | 3.6 |
| 2025-01 | Texas | 3.1 |
| 2025-01 | Florida | 3.3 |
| 2025-01 | New York | 3.6 |
| 2025-01 | Pennsylvania | 3.5 |
| 2025-01 | Illinois | 3.9 |
| 2025-01 | Ohio | 3.6 |
| 2025-01 | Georgia | 3.6 |
| 2025-01 | North Carolina | 3.7 |
| 2025-01 | Michigan | 4.8 |
| 2025-02 | California | 3.7 |
| 2025-02 | Texas | 3.1 |
| 2025-02 | Florida | 3.3 |
| 2025-02 | New York | 3.5 |
| 2025-02 | Pennsylvania | 3.4 |
| 2025-02 | Illinois | 3.5 |
| 2025-02 | Ohio | 3.6 |
| 2025-02 | Georgia | 3.5 |

*Table shows the first 18 of 100 rows. Full dataset: `data.csv`.*


**CSV settings:** field delimiter — semicolon (`;`); decimal separator — period (`.`). File: `data.csv` (100 rows, 10 series).

### Analytical caption

California peaked at 4.6% in March before easing to 4.0%; Michigan is the only state still above 4.5% in October.

---

© 2026 DataDesk US (training dataset). Not BLS official release.

### Sources

Bureau of Labor Statistics LAUS; 10-state teaching extract.
